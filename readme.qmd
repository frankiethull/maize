...
this is testing a precomputed kernel method using an isolation forest as the kernel. simplifying the predictors into distance measurement then solving for the label. this one is an open issue on the github looking for a cleaner feasible solution. it requires passing and isolation forest that needs run on the in- and out- of sample data after trained on in-. adding a layer of complexity. additionally, it took many times until this worked, changing the isolation.forest prediction mode, using reference points, wrapping as.kernelMatrix() for kernlab to predict properly. 
